...
INTADRS:	EQU 0F908H  ;INT ADRESS POINTER
;INTADRES +2: 090A : NMI POINTER
;STACK POINTER;
;STACKPOINT: EQU 0F7FDH		;2 BYTE ; UNCOMMENT THIS TO RUN IN ROM
STACKPOINT:	EQU 0F000H		; Starting address for stack
...
;	NOP
;	LD A,037H
;	OUT (001H),A	;RESET ALL ERR.FLAGS AND
;				;ENABLE RXRDY,TXRDY (COMMAND)
;	JP GREET
;-------------------------------------------------------------------
...
;RTX_OPCODE:	EQU   01H	;1 BYTE
;OPCODE_PORT:EQU   01H	;1 BYTE
;EXEC_PORT:	EQU   00H	;1 BYTE
;TXD:
;	PUSH	AF			; Store character
;	LD	A,RTX_OPCODE	; A = Serial TX Opcode
;	OUT	(OPCODE_PORT),A	; Write the Serial TX Opcode to IOS
;	POP	AF			; Retrieve character
;	OUT	(EXEC_PORT),A	; Output the character
;	RET
;-----------------------------------------------------------------
...
RXD:
	...
	JR	Z,RXD			; Repeat until RDRF is set
...
;------------------------------------------------------------------
;   UNCOMMENT THIS TO RUN IN ROM
	.ORG 0066H               ;CHANGEABLE NMI INTERRUPT SUBROUTINE
	LD HL,(INTADRS+2)
	JP (HL)
;----------------GREETING MESSAGE----------------------
GREET:
...
BITTI:
	...
	LD (LINELENGHT),A	;SAVE INPUT REGISTER LENGHT TO BC
				...
				;ALL CODE TAKEN INTO INPUT REG.,
				;LENGHT IN LINELENGHT-3FA1H
	LD A,00H
	LD (LINELENGHT + 1), A
...
INTERPRET:		;ASSEMBLER INTERPRETER
	...
	LD HL,(TABFINDAD) 	;MNEMONIC ADRESS FROM TABLE
					;(HOLDS BYTE COUNT AT THE MOMENT)
	LD A,(HL)	;OPERAND COUNT
	LD B,A
	LD C,0 	; CODE COUNT RESET
	INC HL		;HL AT FIRST OPERAND ADRESS(COMMAND)
DISIN:
	LD A,(HL) 	;TAKE COMMAND
...
;------LINE ASSEMLY COMPLETED HERE
	...
	LD (ASADR),HL
YAZIM:
	LD A,(CODEBUFLEN) ;WRITE ASSEMBLY LINE CODES FROM
	LD B,A		;CODE BUFFER TO MEMORYCODE LENGHT
...
;FFC6H :NULDOLNUM		;IF THE OPERAND NULL, BYTE OR
;				;WORD(1 BYTE-2 BYTE) # OR DOLLAR
...
;F900H :NEXTCODE		;NEXT CODE START ADRESS IN DISSASEMBLE SECTOR !
...
ASKDIS: LD A,'D'
	...
	LD (NEXTCODE),HL	;NEXTCODE IS INCREASE LOOP
				;ADRESS FOR DISASSEMBLER
...
DIS51:	INC HL
	...
	LD (OPERAND),HL	;OPERAND1: IF #; FIRST BYTE,
				;IF $; LOW AND HIGH BYTES IN ORDER
...
INTLINR:                   ;4 NIBBLE  INPUT
	...
	LD (KEYINRES + 1),A
INLIN2:
	LD HL, KEYINBUF + 1       ;2 NIBBLE INPUT
	LD A,(HL)
	CALL BYTERDR
	LD (KEYINRES),A
	RET
...
INTLIN:
	...
	LD L,A		;Put in L
	LD A,(HEXLDREG)	;CHECK IF HL RECORDED TO DISPLAY
				; SOURCE ADRESS BEFORE?
...
COMP:
	...
	JP DONGU
MINUS:
	LD A,E      ;E 10H TAN KUCUKSE..
	SUB 10H
	JP C,MINIMUS
	JP DONGU

MINIMUS:
	LD B,E    ;DE 10H TAN AZ
	JP DONGU1
DONGU:
	LD B,10H
DONGU1:
...
HEXOUT:
	PUSH BC ; THIS SUBROUTINE SENDS CONTENTS OF ACC TO TERMINAL AS
	...
	RET
HEXAS:
	LD HL, TABLEX ; HEX NIBBLE TO ASCII CONVERTER
...
;INSTRUCTION , VIRGUL, CODE COUNT,MACHINE CODE,OPERAND #-$-NULL,
;TOTAL BYTE COUNT
;2CH VIRGUL DEMEK
;BU ASSEMBLER .DB DE VIRGUL KABUL ETMIYOR!!!
TABLECOD:
	DB '_ADC A',2CH,'(HL)',1,08EH,0FFH
	DB '_ADC A',2CH,'A',1,08FH,0FFH	;0FFH IS NULL INDEX CODE
							;TO FIND  COMMAND
	DB '_ADC A',2CH,'B',1,088H,0FFH	;'_' IS INDEX CHAR
							;TO FIND MNEMONIC
